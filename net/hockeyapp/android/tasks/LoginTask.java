...
public class LoginTask extends ConnectionTask<Void, Void, Boolean> {
    ...
    private HttpURLConnection makeRequest(int mode, Map<String, String> params) throws IOException {
        ...
        if (mode == 2) {
            return new HttpURLConnectionBuilder(this.mUrlString).setRequestMethod("POST").setBasicAuthorization((String) params.get(Scopes.EMAIL), (String) params.get("password")).build();
        }
        if (mode == 3) {
    ...
    private boolean handleResponse(String responseStr) {
        ...
        try {
            ...
            String status = response.getString(NotificationCompatApi21.CATEGORY_STATUS);
