...
final class AtomParsers {
    ...
    public static Track parseTrak(ContainerAtom trak, LeafAtom mvhd, long duration, boolean isQuickTime) {
        ...
        if (duration == -1) {
            ...
            durationUs = Util.scaleLargeTimestamp(duration, C0747C.MICROS_PER_SECOND, movieTimescale);
        }
    ...
    public static TrackSampleTable parseStbl(Track track, ContainerAtom stblAtom) throws ParserException {
        ...
        if (track.editListDurations == null) {
            Util.scaleLargeTimestampsInPlace(timestamps, C0747C.MICROS_PER_SECOND, track.timescale);
            return new TrackSampleTable(offsets, sizes, maximumSize, timestamps, flags);
        } else if (track.editListDurations.length == 1 && track.editListDurations[0] == 0) {
            for (i = 0; i < timestamps.length; i++) {
                timestamps[i] = Util.scaleLargeTimestamp(timestamps[i] - track.editListMediaTimes[0], C0747C.MICROS_PER_SECOND, track.timescale);
            }
            return new TrackSampleTable(offsets, sizes, maximumSize, timestamps, flags);
        } else {
            long mediaTime;
            ...
            for (i = 0; i < track.editListDurations.length; i++) {
                ...
                if (mediaTime != -1) {
                    ...
                    for (int j = startIndex; j < endIndex; j++) {
                        editedTimestamps[sampleIndex] = Util.scaleLargeTimestamp(pts, C0747C.MICROS_PER_SECOND, track.movieTimescale) + Util.scaleLargeTimestamp(timestamps[j] - mediaTime, C0747C.MICROS_PER_SECOND, track.timescale);
    ...
    private static AvcCData parseAvcCFromParent(ParsableByteArray parent, int position) {
        ...
        float pixelWidthAspectRatio = TouchHelperCallback.ALPHA_FULL;
    ...
    private static Pair<String, byte[]> parseEsdsFromParent(ParsableByteArray parent, int position) {
        ...
        if ((flags & MessagesController.UPDATE_MASK_USER_PHONE) != 0) {
            parent.skipBytes(2);
        }
        if ((flags & 64) != 0) {
    ...
    private static int parseExpandableClassSize(ParsableByteArray data) {
        ...
        while ((currentByte & MessagesController.UPDATE_MASK_USER_PHONE) == MessagesController.UPDATE_MASK_USER_PHONE) {
