...
public class FileUploadOperation {
    ...
    class C16813 implements RequestDelegate {
        C16813() {
        }
    ...
    public void start() {
        if (this.state == 0) {
            this.state = 1;
            Utilities.stageQueue.postRunnable(new C04871());
        }
    }
    ...
    protected void checkNewDataAvailable(long finalSize) {
        Utilities.stageQueue.postRunnable(new C04882(finalSize));
    }
        ...
        if (this.state == 1) {
            try {
                ...
                if (this.stream == null) {
                    ...
                    if (MessagesController.UPDATE_MASK_PHONE % this.uploadChunkSize != 0) {
                        int chunkSize = 64;
                    ...
                    this.uploadChunkSize *= MessagesController.UPDATE_MASK_PHONE;
                    ...
                    if (this.isEncrypted) {
                        try {
                            ...
                            for (a = 0; a < 4; a++) {
                                this.fingerprint |= ((digest[a] ^ digest[a + 4]) & NalUnitUtil.EXTENDED_SAR) << (a * 8);
                            }
                        } catch (Throwable e2) {
                ...
                this.requestToken = ConnectionsManager.getInstance().sendRequest(finalRequest, new C16813(), 0, 4);
            } catch (Throwable e22) {
