...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    /* renamed from: org.telegram.messenger.support.util.MessageThreadUtil.1 */
    class C17241 implements MainThreadCallback<T> {
        ...
        class C08501 implements Runnable {
            ...
            public void run() {
                SyncQueueItem msg = C17241.this.mQueue.next();
                while (msg != null) {
                    switch (msg.what) {
                        case C17241.UPDATE_ITEM_COUNT /*1*/:
                            C17241.this.val$callback.updateItemCount(msg.arg1, msg.arg2);
                            break;
                        case C17241.ADD_TILE /*2*/:
                            C17241.this.val$callback.addTile(msg.arg1, (Tile) msg.data);
                            break;
                        case C17241.REMOVE_TILE /*3*/:
                            C17241.this.val$callback.removeTile(msg.arg1, msg.arg2);
                            break;
                        default:
                            Log.e("ThreadUtil", "Unsupported message, what=" + msg.what);
                            break;
                    }
                    msg = C17241.this.mQueue.next();
                }
            }
        }

        C17241(MainThreadCallback mainThreadCallback) {
            this.val$callback = mainThreadCallback;
            this.mQueue = new MessageQueue();
            this.mMainThreadHandler = new Handler(Looper.getMainLooper());
            this.mMainThreadRunnable = new C08501();
        }
    ...
    /* renamed from: org.telegram.messenger.support.util.MessageThreadUtil.2 */
    class C17252 implements BackgroundCallback<T> {
        ...
        class C08511 implements Runnable {
            ...
            public void run() {
                while (true) {
                    SyncQueueItem msg = C17252.this.mQueue.next();
                    if (msg != null) {
                        switch (msg.what) {
                            case C17252.REFRESH /*1*/:
                                C17252.this.mQueue.removeMessages(C17252.REFRESH);
                                C17252.this.val$callback.refresh(msg.arg1);
                                break;
                            case C17252.UPDATE_RANGE /*2*/:
                                C17252.this.mQueue.removeMessages(C17252.UPDATE_RANGE);
                                C17252.this.mQueue.removeMessages(C17252.LOAD_TILE);
                                C17252.this.val$callback.updateRange(msg.arg1, msg.arg2, msg.arg3, msg.arg4, msg.arg5);
                                break;
                            case C17252.LOAD_TILE /*3*/:
                                C17252.this.val$callback.loadTile(msg.arg1, msg.arg2);
                                break;
                            case C17252.RECYCLE_TILE /*4*/:
                                C17252.this.val$callback.recycleTile((Tile) msg.data);
                                break;
                            default:
                                Log.e("ThreadUtil", "Unsupported message, what=" + msg.what);
                                break;
                    ...
                    C17252.this.mBackgroundRunning.set(false);
                    return;
                }
            }
        }

        C17252(BackgroundCallback backgroundCallback) {
            ...
            this.mBackgroundRunnable = new C08511();
        }
