...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    void layoutChunk(Recycler recycler, State state, LayoutState layoutState, LayoutChunkResult result) {
        ...
        for (i = 0; i < count; i++) {
            ...
            float otherSize = (TouchHelperCallback.ALPHA_FULL * ((float) this.mOrientationHelper.getDecoratedMeasurementInOther(view))) / ((float) lp.mSpanSize);
        ...
        if (flexibleInOtherDir) {
            ...
            for (i = 0; i < count; i++) {
                ...
                spec = LayoutManager.getChildMeasureSpec(this.mCachedBorders[lp.mSpanIndex + lp.mSpanSize] - this.mCachedBorders[lp.mSpanIndex], C0747C.ENCODING_PCM_32BIT, 0, this.mOrientation == 0 ? lp.height : lp.width, DEBUG);
        ...
        int maxMeasureSpec = MeasureSpec.makeMeasureSpec(maxSize, C0747C.ENCODING_PCM_32BIT);
        for (i = 0; i < count; i++) {
            view = this.mSet[i];
    ...
    private int updateSpecWithExtra(int spec, int startInset, int endInset) {
        ...
        if (mode == LinearLayoutManager.INVALID_OFFSET || mode == C0747C.ENCODING_PCM_32BIT) {
