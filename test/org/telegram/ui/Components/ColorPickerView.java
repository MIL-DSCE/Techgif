...
public class ColorPickerView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == C0747C.ENCODING_PCM_32BIT) {
            width = widthSize;
        } else if (widthMode == LinearLayoutManager.INVALID_OFFSET) {
            width = Math.min(intrinsicSize, widthSize);
        } else {
            width = intrinsicSize;
        }
        if (heightMode == C0747C.ENCODING_PCM_32BIT) {
            height = heightSize;
        } else if (heightMode == LinearLayoutManager.INVALID_OFFSET) {
    ...
    private int calculateColor(float angle) {
        ...
        if (unit < 0.0f) {
            unit += TouchHelperCallback.ALPHA_FULL;
        }
        if (unit <= 0.0f) {
            return COLORS[0];
        }
        if (unit >= TouchHelperCallback.ALPHA_FULL) {
