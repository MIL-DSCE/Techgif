...
public class PhoneFormat {
    ...
    public void init(String countryCode) {
        ...
        try {
            ...
            try {
                byte[] buf = new byte[MessagesController.UPDATE_MASK_PHONE];
                while (true) {
                    int len = stream.read(buf, 0, MessagesController.UPDATE_MASK_PHONE);
                    if (len == -1) {
                        break;
                    }
                    bos.write(buf, 0, len);
                ...
                for (int s = 0; s < setCnt; s++) {
                    ...
                    for (int r = 0; r < ruleCnt; r++) {
                        ...
                        if (openPos != -1) {
                            ...
                            r24 = new Object[2];
                            r24[0] = rule.format.substring(0, openPos);
                            r24[1] = rule.format.substring(closePos + 2);
                            rule.format = String.format("%s%s", r24);
                        }
                        arrayList.add(rule);
