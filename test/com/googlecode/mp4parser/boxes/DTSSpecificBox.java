...
public class DTSSpecificBox extends AbstractBox {
    ...
    public int[] getDashAudioChannelConfiguration() {
        ...
        if ((channelLayout & 16) == 16) {
            numChannels++;
            dwChannelMask |= MessagesController.UPDATE_MASK_READ_DIALOG_MESSAGE;
        }
        if ((channelLayout & 32) == 32) {
            numChannels += 2;
            dwChannelMask = (dwChannelMask | MessagesController.UPDATE_MASK_SEND_STATE) | MessagesController.UPDATE_MASK_CHAT_ADMINS;
        }
        if ((channelLayout & 64) == 64) {
            numChannels += 2;
            dwChannelMask = (dwChannelMask | 16) | 32;
        }
        if ((channelLayout & MessagesController.UPDATE_MASK_USER_PHONE) == MessagesController.UPDATE_MASK_USER_PHONE) {
            numChannels++;
            dwChannelMask |= MessagesController.UPDATE_MASK_CHANNEL;
        }
        if ((channelLayout & MessagesController.UPDATE_MASK_READ_DIALOG_MESSAGE) == MessagesController.UPDATE_MASK_READ_DIALOG_MESSAGE) {
            numChannels++;
            dwChannelMask |= MessagesController.UPDATE_MASK_NEW_MESSAGE;
        }
        if ((channelLayout & MessagesController.UPDATE_MASK_SELECT_DIALOG) == MessagesController.UPDATE_MASK_SELECT_DIALOG) {
            numChannels += 2;
            dwChannelMask = (dwChannelMask | 64) | MessagesController.UPDATE_MASK_USER_PHONE;
        }
        if ((channelLayout & MessagesController.UPDATE_MASK_PHONE) == MessagesController.UPDATE_MASK_PHONE) {
            numChannels += 2;
            dwChannelMask = (dwChannelMask | MessagesController.UPDATE_MASK_SELECT_DIALOG) | MessagesController.UPDATE_MASK_PHONE;
        }
        if ((channelLayout & MessagesController.UPDATE_MASK_NEW_MESSAGE) == MessagesController.UPDATE_MASK_NEW_MESSAGE) {
            numChannels += 2;
            dwChannelMask = (dwChannelMask | 16) | 32;
        }
        if ((channelLayout & MessagesController.UPDATE_MASK_SEND_STATE) == MessagesController.UPDATE_MASK_SEND_STATE) {
            numChannels++;
            dwChannelMask |= 8;
        }
        ...
        if ((channelLayout & MessagesController.UPDATE_MASK_CHAT_ADMINS) == MessagesController.UPDATE_MASK_CHAT_ADMINS) {
            numChannels++;
            dwChannelMask |= AccessibilityNodeInfoCompat.ACTION_CUT;
        }
        if ((channelLayout & TLRPC.MESSAGE_FLAG_EDITED) == TLRPC.MESSAGE_FLAG_EDITED) {
            numChannels += 2;
            dwChannelMask = (dwChannelMask | TLRPC.MESSAGE_FLAG_EDITED) | AccessibilityNodeInfoCompat.ACTION_SET_SELECTION;
        }
