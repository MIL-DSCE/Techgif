...
public final class IsoTypeReader {
    ...
    public static int byte2int(byte b) {
        return b < null ? b + MessagesController.UPDATE_MASK_READ_DIALOG_MESSAGE : b;
    }
    ...
    public static double readFixedPoint1616(ByteBuffer bb) {
        ...
        bb.get(bytes);
        return ((double) ((((0 | ((bytes[0] << 24) & ViewCompat.MEASURED_STATE_MASK)) | ((bytes[1] << 16) & 16711680)) | ((bytes[2] << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK)) | (bytes[3] & NalUnitUtil.EXTENDED_SAR))) / 65536.0d;
    }

    public static double readFixedPoint0230(ByteBuffer bb) {
        byte[] bytes = new byte[4];
        bb.get(bytes);
        return ((double) ((((0 | ((bytes[0] << 24) & ViewCompat.MEASURED_STATE_MASK)) | ((bytes[1] << 16) & 16711680)) | ((bytes[2] << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK)) | (bytes[3] & NalUnitUtil.EXTENDED_SAR))) / 1.073741824E9d;
    }

    public static float readFixedPoint88(ByteBuffer bb) {
        byte[] bytes = new byte[2];
        bb.get(bytes);
        return ((float) ((short) ((bytes[1] & NalUnitUtil.EXTENDED_SAR) | ((short) (((bytes[0] << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (short) 0))))) / 256.0f;
    }
