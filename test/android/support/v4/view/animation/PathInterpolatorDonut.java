...
class PathInterpolatorDonut implements Interpolator {
    ...
    public float getInterpolation(float t) {
        ...
        if (t >= TouchHelperCallback.ALPHA_FULL) {
    ...
    private static Path createQuad(float controlX, float controlY) {
        ...
        path.quadTo(controlX, controlY, TouchHelperCallback.ALPHA_FULL, TouchHelperCallback.ALPHA_FULL);
        return path;
    }
    ...
    private static Path createCubic(float controlX1, float controlY1, float controlX2, float controlY2) {
        Path path = new Path();
        path.moveTo(0.0f, 0.0f);
        path.cubicTo(controlX1, controlY1, controlX2, controlY2, TouchHelperCallback.ALPHA_FULL, TouchHelperCallback.ALPHA_FULL);
        return path;
    }
}
