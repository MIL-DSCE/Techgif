...
public class ViewPager extends ViewGroup {
    ...
    static class C00831 implements Comparator<ItemInfo> {
        C00831() {
        }
    ...
    static class C00842 implements Interpolator {
        C00842() {
        }
    ...
    public static class SavedState extends BaseSavedState {
        ...
        static class C15081 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C15081() {
            }
        ...
        static {
            CREATOR = ParcelableCompat.newCreator(new C15081());
        }
    ...
    class C15074 implements OnApplyWindowInsetsListener {
        private final Rect mTempRect;
    ...
    class MyAccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public void onInitializeAccessibilityEvent(View host, AccessibilityEvent event) {
            ...
            if (event.getEventType() == MessagesController.UPDATE_MASK_SEND_STATE && ViewPager.this.mAdapter != null) {
    ...
    static {
        ...
        LAYOUT_ATTRS = iArr;
        COMPARATOR = new C00831();
        sInterpolator = new C00842();
    ...
    public ViewPager(Context context) {
        ...
        this.mEndScrollRunnable = new C00853();
    ...
    void initViewPager() {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(this, new C15074());
    }
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(TouchHelperCallback.ALPHA_FULL, (TouchHelperCallback.ALPHA_FULL * ((float) Math.abs(dx))) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) ((TouchHelperCallback.ALPHA_FULL + (((float) Math.abs(dx)) / (((float) this.mPageMargin) + (((float) width) * this.mAdapter.getPageWidth(this.mCurItem))))) * 100.0f);
        }
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        this.mLastOffset = curItem.position == N + INVALID_POINTER ? (curItem.offset + curItem.widthFactor) - TouchHelperCallback.ALPHA_FULL : AutoScrollHelper.NO_MAX;
        ...
        while (i < itemCount) {
            ...
            if (ii.position == N + INVALID_POINTER) {
                this.mLastOffset = (ii.widthFactor + offset) - TouchHelperCallback.ALPHA_FULL;
            }
            ii.offset = offset;
            offset += ii.widthFactor + marginOffset;
            i += SCROLL_STATE_DRAGGING;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (i = SCROLL_STATE_IDLE; i < size; i += SCROLL_STATE_DRAGGING) {
            ...
            if (child.getVisibility() != 8) {
                ...
                if (lp != null && lp.isDecor) {
                    ...
                    if (consumeVertical) {
                        widthMode = C0747C.ENCODING_PCM_32BIT;
                    } else if (consumeHorizontal) {
                        heightMode = C0747C.ENCODING_PCM_32BIT;
                    }
        ...
        this.mChildWidthMeasureSpec = MeasureSpec.makeMeasureSpec(childWidthSize, C0747C.ENCODING_PCM_32BIT);
        this.mChildHeightMeasureSpec = MeasureSpec.makeMeasureSpec(childHeightSize, C0747C.ENCODING_PCM_32BIT);
        this.mInLayout = true;
        populate();
        ...
        for (i = SCROLL_STATE_IDLE; i < size; i += SCROLL_STATE_DRAGGING) {
            ...
            if (child.getVisibility() != 8) {
                lp = (LayoutParams) child.getLayoutParams();
                if (lp == null || !lp.isDecor) {
                    child.measure(MeasureSpec.makeMeasureSpec((int) (((float) childWidthSize) * lp.widthFactor), C0747C.ENCODING_PCM_32BIT), this.mChildHeightMeasureSpec);
                }
            }
        }
    }
                ...
                if (!lp.isDecor) {
                    ...
                    if (ii != null) {
                        ...
                        if (lp.needsMeasure) {
                            ...
                            int makeMeasureSpec = MeasureSpec.makeMeasureSpec((int) (((float) childWidth) * lp.widthFactor), C0747C.ENCODING_PCM_32BIT);
                            child.measure(widthSpec, MeasureSpec.makeMeasureSpec((height - paddingTop) - paddingBottom, C0747C.ENCODING_PCM_32BIT));
                        }
                        child.layout(childLeft, childTop, child.getMeasuredWidth() + childLeft, child.getMeasuredHeight() + childTop);
                    }
                }
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode == 0 || (overScrollMode == SCROLL_STATE_DRAGGING && this.mAdapter != null && this.mAdapter.getCount() > SCROLL_STATE_DRAGGING)) {
            ...
            if (!this.mRightEdge.isFinished()) {
                ...
                height = (getHeight() - getPaddingTop()) - getPaddingBottom();
                canvas.rotate(90.0f);
                canvas.translate((float) (-getPaddingTop()), (-(this.mLastOffset + TouchHelperCallback.ALPHA_FULL)) * ((float) width));
    ...
    public boolean dispatchPopulateAccessibilityEvent(AccessibilityEvent event) {
        if (event.getEventType() == MessagesController.UPDATE_MASK_SEND_STATE) {
