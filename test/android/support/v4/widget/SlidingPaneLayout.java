...
public class SlidingPaneLayout extends ViewGroup {
        ...
        for (i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() == 8) {
                ...
                if (r0 == -2) {
                    ...
                    i2 = lp.width;
                    if (r0 == -1) {
                        childWidthSpec = MeasureSpec.makeMeasureSpec(widthAvailable - horizontalMargin, C0747C.ENCODING_PCM_32BIT);
                    } else {
                        childWidthSpec = MeasureSpec.makeMeasureSpec(lp.width, C0747C.ENCODING_PCM_32BIT);
                    }
                }
                i2 = lp.height;
                if (r0 == -2) {
                    ...
                    i2 = lp.height;
                    if (r0 == -1) {
                        childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, C0747C.ENCODING_PCM_32BIT);
                    } else {
                        childHeightSpec = MeasureSpec.makeMeasureSpec(lp.height, C0747C.ENCODING_PCM_32BIT);
                    }
                }
                                ...
                                if (canSlide) {
                                    ...
                                    if (child != r0) {
                                        if (lp.width >= 0) {
                                            ...
                                            if (skippedFirstPass) {
                                                childHeightSpec = MeasureSpec.makeMeasureSpec(child.getMeasuredHeight(), C0747C.ENCODING_PCM_32BIT);
                                            } else {
                                                i2 = lp.height;
                                                if (r0 != -2) {
                                                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, LinearLayoutManager.INVALID_OFFSET);
                                            ...
                                            child.measure(MeasureSpec.makeMeasureSpec(fixedPanelWidthLimit, C0747C.ENCODING_PCM_32BIT), childHeightSpec);
                                        }
                                    }
                                }
                                if (lp.weight <= 0.0f) {
                                    ...
                                    if (canSlide) {
                                        child.measure(MeasureSpec.makeMeasureSpec(measuredWidth + ((int) ((lp.weight * ((float) Math.max(0, widthRemaining))) / weightSum)), C0747C.ENCODING_PCM_32BIT), childHeightSpec);
                                    } else {
                                        newWidth = widthAvailable - (lp.leftMargin + lp.rightMargin);
                                        childWidthSpec = MeasureSpec.makeMeasureSpec(newWidth, C0747C.ENCODING_PCM_32BIT);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (this.mFirstLayout) {
            float f = (this.mCanSlide && this.mPreservedOpenState) ? TouchHelperCallback.ALPHA_FULL : 0.0f;
            this.mSlideOffset = f;
        }
        for (i = 0; i < childCount; i++) {
            View child = getChildAt(i);
            if (child.getVisibility() != 8) {
                ...
                if (lp.slideable) {
                    ...
                    offset = (int) ((TouchHelperCallback.ALPHA_FULL - this.mSlideOffset) * ((float) this.mParallaxBy));
                    xStart = nextXStart;
                }
                if (isLayoutRtl) {
    ...
    private boolean openPane(View pane, int initialVelocity) {
        if (!this.mFirstLayout && !smoothSlideTo(TouchHelperCallback.ALPHA_FULL, initialVelocity)) {
            return false;
        }
    ...
    public boolean isOpen() {
        return !this.mCanSlide || this.mSlideOffset == TouchHelperCallback.ALPHA_FULL;
    }
    ...
    private void parallaxOtherViews(float slideOffset) {
            ...
            if ((isLayoutRtl ? slideLp.rightMargin : slideLp.leftMargin) <= 0) {
                ...
                for (i = 0; i < childCount; i++) {
                    ...
                    if (v == this.mSlideableView) {
                        int oldOffset = (int) ((TouchHelperCallback.ALPHA_FULL - this.mParallaxOffset) * ((float) this.mParallaxBy));
                        this.mParallaxOffset = slideOffset;
                        dx = oldOffset - ((int) ((TouchHelperCallback.ALPHA_FULL - slideOffset) * ((float) this.mParallaxBy)));
                        if (isLayoutRtl) {
                            dx = -dx;
                        }
                        v.offsetLeftAndRight(dx);
                        if (!dimViews) {
                            dimChildView(v, isLayoutRtl ? this.mParallaxOffset - TouchHelperCallback.ALPHA_FULL : TouchHelperCallback.ALPHA_FULL - this.mParallaxOffset, this.mCoveredFadeColor);
                        }
                    }
                }
            }
        ...
        for (i = 0; i < childCount; i++) {
            v = getChildAt(i);
            if (v == this.mSlideableView) {
                int oldOffset2 = (int) ((TouchHelperCallback.ALPHA_FULL - this.mParallaxOffset) * ((float) this.mParallaxBy));
                this.mParallaxOffset = slideOffset;
                dx = oldOffset2 - ((int) ((TouchHelperCallback.ALPHA_FULL - slideOffset) * ((float) this.mParallaxBy)));
                if (isLayoutRtl) {
                    dx = -dx;
                }
                v.offsetLeftAndRight(dx);
