    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.mFillViewport && MeasureSpec.getMode(heightMeasureSpec) != 0 && getChildCount() > 0) {
            ...
            if (child.getMeasuredHeight() < height) {
                child.measure(getChildMeasureSpec(widthMeasureSpec, getPaddingLeft() + getPaddingRight(), ((FrameLayout.LayoutParams) child.getLayoutParams()).width), MeasureSpec.makeMeasureSpec((height - getPaddingTop()) - getPaddingBottom(), C0747C.ENCODING_PCM_32BIT));
            }
        }
    }
                        ...
                        if (!dispatchNestedScroll(0, scrolledDeltaY, 0, deltaY - scrolledDeltaY, this.mScrollOffset)) {
                            if (canOverscroll) {
                                ...
                                if (pulledToY < 0) {
                                    ...
                                    this.mEdgeGlowBottom.onPull(((float) deltaY) / ((float) getHeight()), TouchHelperCallback.ALPHA_FULL - (MotionEventCompat.getX(ev, activePointerIndex) / ((float) getWidth())));
