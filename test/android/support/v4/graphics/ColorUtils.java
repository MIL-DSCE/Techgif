...
public final class ColorUtils {
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int r, @IntRange(from = 0, to = 255) int g, @IntRange(from = 0, to = 255) int b, @NonNull float[] outHsl) {
        ...
        if (max == min) {
            ...
            s = deltaMaxMin / (TouchHelperCallback.ALPHA_FULL - Math.abs((2.0f * l) - TouchHelperCallback.ALPHA_FULL));
        }
        ...
        outHsl[MIN_ALPHA_SEARCH_PRECISION] = constrain(s, 0.0f, (float) TouchHelperCallback.ALPHA_FULL);
        outHsl[2] = constrain(l, 0.0f, (float) TouchHelperCallback.ALPHA_FULL);
    }
    ...
    public static int HSLToColor(@NonNull float[] hsl) {
        ...
        float l = hsl[2];
        float c = (TouchHelperCallback.ALPHA_FULL - Math.abs((2.0f * l) - TouchHelperCallback.ALPHA_FULL)) * s;
        float m = l - (0.5f * c);
        float x = c * (TouchHelperCallback.ALPHA_FULL - Math.abs(((h / BitmapDescriptorFactory.HUE_YELLOW) % 2.0f) - TouchHelperCallback.ALPHA_FULL));
        int r = 0;
        int g = 0;
        int b = 0;
        switch (((int) h) / 60) {
    ...
    public static int blendARGB(@ColorInt int color1, @ColorInt int color2, @FloatRange(from = 0.0d, to = 1.0d) float ratio) {
        float inverseRatio = TouchHelperCallback.ALPHA_FULL - ratio;
