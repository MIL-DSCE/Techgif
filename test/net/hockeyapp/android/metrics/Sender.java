...
public class Sender {
    ...
    class C03961 extends AsyncTask<Void, Void, Void> {
        C03961() {
        }
    ...
    /* renamed from: net.hockeyapp.android.metrics.Sender.2 */
    class C03972 extends AsyncTask<Void, Void, Void> {
        final /* synthetic */ HttpURLConnection val$connection;
        final /* synthetic */ File val$file;
        final /* synthetic */ String val$persistedData;

        C03972(HttpURLConnection httpURLConnection, File file, String str) {
            this.val$connection = httpURLConnection;
            this.val$file = file;
            this.val$persistedData = str;
        }

        protected Void doInBackground(Void... params) {
            Sender.this.send(this.val$connection, this.val$file, this.val$persistedData);
            return null;
        }
    }
    ...
    protected void triggerSending() {
        if (requestCount() < MAX_REQUEST_COUNT) {
            ...
            AsyncTaskUtils.execute(new C03961());
            return;
        }
        HockeyLog.debug(TAG, "We have already 10 pending requests, not sending anything.");
    }

    protected void triggerSendingForTesting(HttpURLConnection connection, File file, String persistedData) {
        if (requestCount() < MAX_REQUEST_COUNT) {
            this.mRequestCount.getAndIncrement();
            AsyncTaskUtils.execute(new C03972(connection, file, persistedData));
        }
    }
    ...
    protected void readResponse(HttpURLConnection connection, StringBuilder builder) {
        ...
        try {
            ...
            if (inputStream != null) {
                BufferedReader br = new BufferedReader(new InputStreamReader(inputStream, C0747C.UTF8_NAME));
    ...
    protected Writer getWriter(HttpURLConnection connection) throws IOException {
        if (VERSION.SDK_INT < 19) {
            return new OutputStreamWriter(connection.getOutputStream(), C0747C.UTF8_NAME);
        }
        connection.addRequestProperty("Content-Encoding", "gzip");
        connection.setRequestProperty("Content-Type", "application/x-json-stream");
        return new OutputStreamWriter(new GZIPOutputStream(connection.getOutputStream(), true), C0747C.UTF8_NAME);
    }
