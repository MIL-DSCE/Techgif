...
public class Constants {
    ...
    public static File getHockeyAppStorageDir() {
        File dir = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/" + FILES_DIRECTORY_NAME);
    ...
    private static int loadBuildNumber(Context context, PackageManager packageManager) {
        int i = 0;
        try {
            Bundle metaData = packageManager.getApplicationInfo(context.getPackageName(), MessagesController.UPDATE_MASK_USER_PHONE).metaData;
            if (metaData != null) {
    ...
    private static void loadCrashIdentifier(Context context) {
        ...
        if (!TextUtils.isEmpty(APP_PACKAGE) && !TextUtils.isEmpty(deviceIdentifier)) {
            ...
            try {
                ...
                byte[] bytes = combined.getBytes(C0747C.UTF8_NAME);
    ...
    private static String bytesToHex(byte[] bytes) {
        ...
        for (int index = 0; index < bytes.length; index += UPDATE_PERMISSIONS_REQUEST) {
            int value = bytes[index] & NalUnitUtil.EXTENDED_SAR;
            hex[index * 2] = HEX_ARRAY[value >>> 4];
            hex[(index * 2) + UPDATE_PERMISSIONS_REQUEST] = HEX_ARRAY[value & 15];
        }
