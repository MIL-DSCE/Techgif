...
public class Base64 {
            ...
            while (p < len) {
                if (state == 0) {
                    while (p + Base64.CRLF <= len) {
                        value = (((alphabet[input[p] & NalUnitUtil.EXTENDED_SAR] << 18) | (alphabet[input[p + Base64.NO_PADDING] & NalUnitUtil.EXTENDED_SAR] << 12)) | (alphabet[input[p + Base64.NO_WRAP] & NalUnitUtil.EXTENDED_SAR] << 6)) | alphabet[input[p + 3] & NalUnitUtil.EXTENDED_SAR];
                        if (value >= 0) {
                ...
                int d = alphabet[input[p] & NalUnitUtil.EXTENDED_SAR];
                switch (state) {
                    case Base64.DEFAULT /*0*/:
                        if (d < 0) {
                            if (d == SKIP) {
    ...
    static class Encoder extends Coder {
        ...
        public Encoder(int flags, byte[] output) {
            ...
            boolean z2 = true;
            this.output = output;
            this.do_padding = (flags & Base64.NO_PADDING) == 0;
