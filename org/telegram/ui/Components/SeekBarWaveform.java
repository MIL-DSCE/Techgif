...
public class SeekBarWaveform {
    ...
    public boolean onTouch(int action, float x, float y) {
        if (action == 0) {
            ...
            if (this.startX == GroundOverlayOptions.NO_DIMENSION || Math.abs(x - this.startX) <= AndroidUtilities.getPixelsInCM(DefaultLoadControl.DEFAULT_LOW_BUFFER_LOAD, true)) {
                return true;
            }
            ...
            this.startX = GroundOverlayOptions.NO_DIMENSION;
            return true;
        }
        return false;
    }
                    ...
                    if (a == nextBarNum) {
                        ...
                        for (int b = 0; b < drawBarCount; b++) {
                            ...
                            if (x >= this.thumbX || AndroidUtilities.dp(2.0f) + x >= this.thumbX) {
                                canvas.drawRect((float) x, (float) (AndroidUtilities.dp(14.0f - Math.max(TouchHelperCallback.ALPHA_FULL, (14.0f * ((float) value)) / 31.0f)) + y), (float) (AndroidUtilities.dp(2.0f) + x), (float) (AndroidUtilities.dp(14.0f) + y), paintInner);
                                if (x < this.thumbX) {
                                    canvas.drawRect((float) x, (float) (AndroidUtilities.dp(14.0f - Math.max(TouchHelperCallback.ALPHA_FULL, (14.0f * ((float) value)) / 31.0f)) + y), (float) this.thumbX, (float) (AndroidUtilities.dp(14.0f) + y), paintOuter);
                                }
                            } else {
                                canvas.drawRect((float) x, (float) (AndroidUtilities.dp(14.0f - Math.max(TouchHelperCallback.ALPHA_FULL, (14.0f * ((float) value)) / 31.0f)) + y), (float) (AndroidUtilities.dp(2.0f) + x), (float) (AndroidUtilities.dp(14.0f) + y), paintOuter);
                            }
                            barNum++;
                        }
                    }
