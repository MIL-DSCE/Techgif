...
public class Scroller {
    ...
    static {
        ...
        START_TENSION = 0.4f;
        END_TENSION = TouchHelperCallback.ALPHA_FULL - START_TENSION;
        ...
        for (int i = 0; i <= NB_SAMPLES; i += FLING_MODE) {
            ...
            float x_max = TouchHelperCallback.ALPHA_FULL;
            while (true) {
                x = x_min + ((x_max - x_min) / 2.0f);
                coef = (3.0f * x) * (TouchHelperCallback.ALPHA_FULL - x);
                float tx = ((((TouchHelperCallback.ALPHA_FULL - x) * START_TENSION) + (END_TENSION * x)) * coef) + ((x * x) * x);
        ...
        SPLINE[NB_SAMPLES] = TouchHelperCallback.ALPHA_FULL;
        sViscousFluidScale = 8.0f;
        sViscousFluidNormalize = TouchHelperCallback.ALPHA_FULL;
        sViscousFluidNormalize = TouchHelperCallback.ALPHA_FULL / viscousFluid(TouchHelperCallback.ALPHA_FULL);
    }
    ...
    public void startScroll(int startX, int startY, int dx, int dy, int duration) {
        ...
        this.mDurationReciprocal = TouchHelperCallback.ALPHA_FULL / ((float) this.mDuration);
    }
    ...
    public void fling(int startX, int startY, int velocityX, int velocityY, int minX, int maxX, int minY, int maxY) {
        ...
        this.mStartY = startY;
        if (velocity == 0.0f) {
            coeffX = TouchHelperCallback.ALPHA_FULL;
        } else {
            coeffX = ((float) velocityX) / velocity;
        }
        if (velocity == 0.0f) {
            coeffY = TouchHelperCallback.ALPHA_FULL;
        } else {
    ...
    static float viscousFluid(float x) {
        ...
        if (x < TouchHelperCallback.ALPHA_FULL) {
            x -= TouchHelperCallback.ALPHA_FULL - ((float) Math.exp((double) (-x)));
        } else {
            x = 0.36787945f + ((TouchHelperCallback.ALPHA_FULL - 0.36787945f) * (TouchHelperCallback.ALPHA_FULL - ((float) Math.exp((double) (TouchHelperCallback.ALPHA_FULL - x)))));
        }
