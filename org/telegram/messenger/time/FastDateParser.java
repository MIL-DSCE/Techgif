...
public class FastDateParser implements DateParser, Serializable {
    ...
    private static class TimeZoneStrategy extends Strategy {
        ...
        TimeZoneStrategy(Locale locale) {
            ...
            String[][] arr$ = DateFormatSymbols.getInstance(locale).getZoneStrings();
            int len$ = arr$.length;
            for (int i$ = ID; i$ < len$; i$ += LONG_STD) {
                String[] zone = arr$[i$];
                if (!zone[ID].startsWith("GMT")) {
                    TimeZone tz = TimeZone.getTimeZone(zone[ID]);
                    if (!this.tzNames.containsKey(zone[LONG_STD])) {
                        this.tzNames.put(zone[LONG_STD], tz);
                    }
                    if (!this.tzNames.containsKey(zone[SHORT_STD])) {
                        this.tzNames.put(zone[SHORT_STD], tz);
                    }
                    if (tz.useDaylightTime()) {
                        if (!this.tzNames.containsKey(zone[LONG_DST])) {
                            this.tzNames.put(zone[LONG_DST], tz);
                        }
                        if (!this.tzNames.containsKey(zone[SHORT_DST])) {
                            this.tzNames.put(zone[SHORT_DST], tz);
                        }
                    }
                }
            }
        ...
        void setCalendar(FastDateParser parser, Calendar cal, String value) {
            TimeZone tz;
            if (value.charAt(ID) == '+' || value.charAt(ID) == '-') {
    ...
    static class C20081 extends NumberStrategy {
        C20081(int x0) {
            super(x0);
        }
    ...
    static class C20092 extends NumberStrategy {
        C20092(int x0) {
            super(x0);
        }
    ...
    static class C20103 extends NumberStrategy {
        C20103(int x0) {
            super(x0);
        }
    ...
    static class C20114 extends NumberStrategy {
        C20114(int x0) {
            super(x0);
        }
    ...
    static {
        ...
        ABBREVIATED_YEAR_STRATEGY = new C20081(1);
        NUMBER_MONTH_STRATEGY = new C20092(2);
        ...
        MODULO_HOUR_OF_DAY_STRATEGY = new C20103(11);
        MODULO_HOUR_STRATEGY = new C20114(10);
