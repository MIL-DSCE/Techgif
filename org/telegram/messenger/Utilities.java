...
public class Utilities {
    ...
    public static String bytesToHex(byte[] bytes) {
        ...
        for (int j = 0; j < bytes.length; j++) {
            int v = bytes[j] & NalUnitUtil.EXTENDED_SAR;
    ...
    public static boolean isGoodPrime(byte[] prime, int g) {
        boolean z = true;
        if (g < 2 || g > 7 || prime.length != MessagesController.UPDATE_MASK_READ_DIALOG_MESSAGE || prime[0] >= null) {
            return false;
        }
    ...
    public static String MD5(String md5) {
        ...
        if (md5 != null) {
            try {
                ...
                for (byte b : array) {
                    sb.append(Integer.toHexString((b & NalUnitUtil.EXTENDED_SAR) | MessagesController.UPDATE_MASK_READ_DIALOG_MESSAGE).substring(1, 3));
                }
                str = sb.toString();
            } catch (Throwable e) {
                FileLog.m13e("tmessages", e);
