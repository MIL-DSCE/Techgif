...
public class ApplicationLoader extends Application {
    ...
    static class C04261 implements Runnable {
        C04261() {
        }
                ...
                try {
                    ...
                    if (selectedColor == 0) {
                        if (selectedBackground == 1000001) {
                            ApplicationLoader.cachedWallpaper = ApplicationLoader.applicationContext.getResources().getDrawable(C0691R.drawable.background_hd);
        ...
        public void run() {
            if (ApplicationLoader.this.checkPlayServices()) {
                if (UserConfig.pushString == null || UserConfig.pushString.length() == 0) {
                    FileLog.m10d("tmessages", "GCM Registration not found.");
                } else {
                    FileLog.m10d("tmessages", "GCM regId = " + UserConfig.pushString);
                }
                ApplicationLoader.this.startService(new Intent(ApplicationLoader.applicationContext, GcmRegistrationIntentService.class));
                return;
            }
            FileLog.m10d("tmessages", "No valid Google Play Services APK found.");
        }
    }
    ...
    public static void loadWallpaper() {
        if (cachedWallpaper == null) {
            Utilities.searchQueue.postRunnable(new C04261());
        }
    }
    ...
    private static void convertConfig() {
        ...
        if (preferences.contains("currentDatacenterId")) {
            boolean z;
            SerializedData buffer = new SerializedData((int) TLRPC.MESSAGE_FLAG_EDITED);
            buffer.writeInt32(2);
    ...
    public static void postInitApplication() {
        if (!applicationInited) {
            ...
            try {
                ...
                FileLog.m11e("tmessages", "screen state = " + isScreenOn);
            } catch (Throwable e3) {
            ...
            ConnectionsManager.getInstance().init(BuildVars.BUILD_VERSION, 53, BuildVars.APP_ID, deviceModel, systemVersion, appVersion, langCode, configPath, FileLog.getNetworkLogPath(), UserConfig.getClientUserId(), enablePushConnection);
            ...
            FileLog.m11e("tmessages", "app initied");
    ...
    public static void stopPushService() {
        ...
        ((AlarmManager) applicationContext.getSystemService(NotificationCompatApi21.CATEGORY_ALARM)).cancel(PendingIntent.getService(applicationContext, 0, new Intent(applicationContext, NotificationsService.class), 0));
    }
    ...
    private void initPlayServices() {
        AndroidUtilities.runOnUIThread(new C04272(), 1000);
    }
