...
public interface HttpDataSource extends UriDataSource {
    ...
    /* renamed from: org.telegram.messenger.exoplayer.upstream.HttpDataSource.1 */
    static class C17001 implements Predicate<String> {
        C17001() {
        }

        public boolean evaluate(String contentType) {
            contentType = Util.toLowerInvariant(contentType);
            return (TextUtils.isEmpty(contentType) || ((contentType.contains(QuicktimeTextSampleEntry.TYPE) && !contentType.contains(MimeTypes.TEXT_VTT)) || contentType.contains("html") || contentType.contains("xml"))) ? false : true;
        }
    }
    ...
    static {
        REJECT_PAYWALL_TYPES = new C17001();
    }
}
