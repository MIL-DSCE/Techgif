...
public class M4AInfo extends AudioInfo {
    ...
    void data(MP4Atom atom) throws IOException {
        ...
        switch (obj) {
            case VideoPlayer.TRACK_DEFAULT /*0*/:
                this.album = atom.readString(UTF_8);
            case MediaCodecVideoTrackRenderer.MSG_SET_SURFACE /*1*/:
                this.albumArtist = atom.readString(UTF_8);
            case MediaCodecAudioTrackRenderer.MSG_SET_PLAYBACK_PARAMS /*2*/:
                this.artist = atom.readString(UTF_8);
            case VideoPlayer.STATE_BUFFERING /*3*/:
                this.comment = atom.readString(UTF_8);
            case VideoPlayer.STATE_READY /*4*/:
            case VideoPlayer.STATE_ENDED /*5*/:
                if (this.composer == null || this.composer.trim().length() == 0) {
                    this.composer = atom.readString(UTF_8);
                }
            ...
            case ConnectionResult.SERVICE_INVALID /*9*/:
                if (this.copyright == null || this.copyright.trim().length() == 0) {
                    this.copyright = atom.readString(UTF_8);
                }
            ...
            case Atom.FULL_HEADER_SIZE /*12*/:
                ...
                this.genre = atom.readString(UTF_8);
            case ConnectionResult.CANCELED /*13*/:
                if (this.genre == null || this.genre.trim().length() == 0) {
                    this.genre = atom.readString(UTF_8);
                }
            case ConnectionResult.TIMEOUT /*14*/:
                this.grouping = atom.readString(UTF_8);
            case ConnectionResult.INTERRUPTED /*15*/:
                this.lyrics = atom.readString(UTF_8);
            case ItemTouchHelper.START /*16*/:
                this.title = atom.readString(UTF_8);
