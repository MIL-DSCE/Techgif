...
public class CencEncryptingTrackImpl implements CencEncryptedTrack {
    ...
    class C03621 extends HashMap<GroupEntry, long[]> {
        C03621(Map $anonymous0) {
            super($anonymous0);
        }
    ...
    public CencEncryptingTrackImpl(Track source, UUID defaultKeyId, Map<UUID, SecretKey> keys, Map<CencSampleEncryptionInformationGroupEntry, long[]> keyRotation, String encryptionAlgo, boolean dummyIvs, boolean encryptButAllClear) {
        ...
        this.sampleGroups = new C03621(this.sampleGroups);
        ...
        for (i = 0; i < source.getSamples().size(); i++) {
            ...
            for (int j = 0; j < groupEntries.size(); j++) {
                ...
                if (Arrays.binarySearch((long[]) getSampleGroups().get(groupEntry), (long) i) >= 0) {
                    index = j + 1;
                }
            }
            if (lastSampleGroupDescriptionIndex != index) {
                if (index == 0) {
                    this.indexToKey.put((Comparable) Integer.valueOf(i), (SecretKey) keys.get(defaultKeyId));
                } else {
                    if (((CencSampleEncryptionInformationGroupEntry) groupEntries.get(index - 1)).getKid() != null) {
                        ...
                        this.indexToKey.put(Integer.valueOf(i), (Object) sk);
                    } else {
    ...
    public synchronized SampleDescriptionBox getSampleDescriptionBox() {
        if (this.stsd == null) {
            ...
            try {
                ...
                schm.setSchemeVersion(AccessibilityNodeInfoCompat.ACTION_CUT);
                sinf.addBox(schm);
